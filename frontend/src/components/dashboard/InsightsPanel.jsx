...
const InsightsPanel = ({ insights, operationalInsights }) => {
  ...
  const InsightCard = ({ insight }) => {
    ...
    if (insight.type === 'critical') {
      IconCmp = AlertCircle; bgColor = 'bg-red-50'; textColor = 'text-red-900'; iconColor = 'text-red-500';
    } else if (insight.type === 'warning') {
      IconCmp = AlertCircle; bgColor = 'bg-yellow-50'; textColor = 'text-yellow-900'; iconColor = 'text-yellow-600';
    } else if (insight.type === 'positive') {
      IconCmp = CheckCircle2; bgColor = 'bg-green-50'; textColor = 'text-green-900'; iconColor = 'text-green-500';
    ...
    <div className="business-card p-8 flex flex-col h-full space-y-8">
      ...
      <div>
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-sm font-bold text-gray-800 uppercase tracking-wider">{t('dashboard.insights.financial')}</h3>
        </div>
        <div className="space-y-3">
          {financialInsights.length > 0
            ? financialInsights.map((i, idx) => <InsightCard key={idx} insight={i} />)
            : <p className="text-[10px] text-gray-400 italic">{t('dashboard.insights.noFinancial')}</p>}
        </div>
      </div>
      ...
      <div>
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-sm font-bold text-gray-800 uppercase tracking-wider">{t('dashboard.insights.operational')}</h3>
        </div>
        <div className="space-y-3">
          {opInsightsArray.length > 0
            ? opInsightsArray.map((i, idx) => <InsightCard key={idx} insight={i} />)
            : <p className="text-[10px] text-gray-400 italic">{t('dashboard.insights.noOperational')}</p>}
        </div>
      </div>
    </div>
  );
