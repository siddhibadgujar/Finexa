      ...
      <div className="bg-white p-4 border border-gray-100 shadow-xl rounded-xl">
        ...
        <div className="flex items-center gap-3 mb-1">
          <div className={`w-3 h-3 rounded-full ${
            isResolved ? 'bg-green-500' : data.type === 'income' ? 'bg-green-500' : 'bg-red-500 shadow-sm shadow-red-200'
          }`} />
        ...
        <div className="flex items-center justify-between gap-4 mt-1">
          ...
          {data.anomaly === 1 && (
            <span className={`text-[10px] font-black px-2 py-0.5 rounded uppercase ${
              isResolved ? 'text-green-600 bg-green-50' : 'text-red-600 bg-red-50'
            }`}>
              {isResolved ? '✓ Resolved' : 'Anomaly'}
            </span>
          )}
        </div>
        {data.explanation && data.anomaly === 1 && !isResolved && (
        ...
        {isResolved && (
          <div className="mt-2 pt-2 border-t border-gray-50">
            <p className="text-[10px] font-black text-green-600 uppercase tracking-widest">✓ Strategy Implemented</p>
          </div>
        )}
      </div>
    );
...
const RenderCustomDot = (props) => {
  ...
  if (payload.anomaly === 1) {
    if (payload.isResolved) {
      // Green dot for resolved anomalies
      return (
        <g key={`dot-${payload.date}`}>
          <circle cx={cx} cy={cy} r={7} fill="#22c55e" fillOpacity={0.2} />
          <circle cx={cx} cy={cy} r={5} fill="#22c55e" stroke="#fff" strokeWidth={2} />
        </g>
      );
    }
    // Red pulsing dot for active anomalies
    return (
      <g key={`dot-${payload.date}`}>
        <circle cx={cx} cy={cy} r={8} fill="#ef4444" fillOpacity={0.2} />
        <circle cx={cx} cy={cy} r={5} fill="#ef4444" stroke="#fff" strokeWidth={2} className="drop-shadow-md" />
      </g>
    );
  }
    ...
    <div className="w-full h-[450px] bg-white p-6 md:p-8 rounded-[2.5rem] border-2 border-gray-50 shadow-xl shadow-indigo-50/20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-2">
            <div className="w-2.5 h-2.5 rounded-full bg-red-500 shadow-sm shadow-red-100 animate-pulse" />
            <span className="text-[10px] font-black text-red-600 uppercase tracking-widest">Active Anomaly</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-2.5 h-2.5 rounded-full bg-green-500 shadow-sm shadow-green-100" />
        ...
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={sortedData} margin={{ top: 10, right: 10, left: 10, bottom: 0 }}>
            ...
            <XAxis
              ...
              stroke="#cbd5e1" fontSize={10} fontWeight="900"
              tickLine={false} axisLine={false} dy={15}
            />
            <YAxis
              stroke="#cbd5e1" fontSize={10} fontWeight="900"
            ...
            />
            <Line
              type="monotone" dataKey="amount" stroke="#6366f1" strokeWidth={4}
              dot={<RenderCustomDot />}
              activeDot={{ r: 7, strokeWidth: 2, stroke: '#fff', fill: '#4f46e5' }}
              connectNulls isAnimationActive animationDuration={1000}
            />
          </LineChart>
        </ResponsiveContainer>
      </div>
