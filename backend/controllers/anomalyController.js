...
exports.detectAnomalies = async (req, res) => {
  try {
    ...
    try {
      ...
      // Advanced Analytics: Severity & Explanations
      ...
      let riskVal   = (highSeverity.length * 30) + (medSeverity.length * 20) + (activeAnomalies.length * 10);
      ...
      res.status(200).json({
        message: activeAnomalies.length > 0 ? 'Anomalies detected' : 'No anomalies detected',
        anomalies: results,   // full list with isResolved flag for graph
      ...
      res.status(503).json({ message: 'ML Service is currently unavailable', error: error.message });
    }
